...
from .rentSelectedRoom import RentSelectedRoomContacts

class ContactsBase(SQLModel):
    contacts_duration : ContactsDuration = Relationship(back_populates=ContactsDuration.contacts)
    contacts_status : ContactsStatus = Relationship(back_populates=ContactsStatus.contacts)
...
class Contacts(ContactsBase, table=True):
    ...
    room_rent = Relationship(back_populates=RentSelectedRoomContacts.contact)
